...
vec4 sample_texture_with_fallback(uint tex_id) {
    if (tex_id == 0xffffffff) {
        return vec4(1, 1, 1, 1);
    }

    return texture(bindless_samplers[tex_id], fragTexCoord);
}
...
void main() {
    ...
    vec4 base_color = sample_texture_with_fallback(base_color_id);

    if (base_color.a < 0.1) discard;

    vec3 normal = sample_texture_with_fallback(normal_id).rgb;
    normal = normalize(normal * 2.0 - 1.0);
    normal = normalize(TBN * normal);

    vec3 orm = sample_texture_with_fallback(orm_id).rgb;
